...
module HockeyApp
  class Client
    ...
    def new_app title, bundle_id, options = {}
      ...
      raise "Image not found at #{icon_path}" if !File.exists? icon_path
      raise "Image format with #{File.extname(icon_path)} extension not supported" if !accepted_formats.include? File.extname(icon_path)
